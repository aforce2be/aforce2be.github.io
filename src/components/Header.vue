<template>
  <header
    :class="[
      isScrolled ? 'shadow' : '',
      'bg-white fixed top-0 left-0 w-full transition-colors duration-300 z-50',
    ]"
  >
    <nav
      class="max-w-7xl mx-auto flex justify-between items-center py-4 max-md:px-4"
    >
    ...
    >
      ...
      <a @click="isOpen = false" :class="linkClass('about')" href="#about"
        >About</a
      >
      <a @click="isOpen = false" :class="linkClass('projects')" href="#projects"
        >Projects</a
      >
      <a @click="isOpen = false" :class="linkClass('contact')" href="#contact"
        >Contact</a
      >
    </div>
  </header>
